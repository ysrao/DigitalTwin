---
title: "Centralized PPO versus Cooperative Multi-Agent Reinforcement Learning for SLA-Protected 5G/6G RAN Slice Allocation (v7)"
authors:
  - name: "Rao Yenamandra"
...
This work isolates one sub-problem of a larger multi-tier 6G RAN digital twin in order to test one architectural hypothesis cheaply. It does not replace the full twin's propagation, mobility, association, handover, interference, non-terrestrial, or multi-access-steering evaluation.
...
Every number in this paper is produced by a synthetic reference implementation of the bounded single-cell PRB-share environment of Section III and the two controllers of Section IV, run under the fixed protocol of Section V: PPO clipping $\epsilon = 0.2$, discount $\gamma = 0.97$, GAE, 260 episodes of 48 steps (12,480 interactions) per training seed, training seeds 1301--1310, and 40 held-out traffic seeds per trained pair. The environment, both controller architectures, the deterministic coordinator, the reward of (1), the traffic generator, and the training-seed-clustered bootstrap are all specified here in full so the experiment can be re-implemented from the paper alone. The reference implementation and the paired-evaluation sweep script are available from the authors on request and are intended for release alongside the full-twin integration described above. No results are drawn from any external dataset, prior publication, or third-party model; the traffic traces and channel-efficiency indicators are synthetic and generated by the reference implementation.
