...
**Algorithmic validity.** Both controllers reuse the same network class and PPO update, which improves comparability. But three implementation choices were ours to make, not fixed by any external protocol, and each could plausibly change the result. First, each MARL agent picks one of 5 fixed bid levels (0.1, 0.3, 0.5, 0.7, 0.9) rather than a true continuous value, because the shared network has a categorical output and cannot produce an arbitrary real number; a genuinely continuous bid might let an agent respond more precisely to demand. Second, six of each agent's seven observed inputs had an obvious source (its own demand, spectral efficiency, previous share, the shared load and availability signals, and the episode phase); the seventh -- that slice's own current SLA shortfall -- was a judgment call, not a specified requirement, and a different seventh input could give the agent different information to act on. Third, the rule that turns the three agents' bids into one feasible allocation -- give each slice its guaranteed floor, then split what remains in proportion to how far above its own floor each bid sits -- is one reasonable design, not the only one; a different combination rule could route resources differently. A different choice on any of these three points could change which architecture converges more reliably, and so could shift this paper's result.
...
**"Held-out" data is not independently generated traffic.** The traffic trace is 4,320 rows; each training run advances through it sequentially, wrapping around, so 260 episodes of 48 steps sweeps the entire trace roughly 2.9 times before training stops. The 40 evaluation windows (seeds 5001--5040) are different starting offsets into that *same fixed trace*, not independently sampled traffic the model has never encountered. Evaluation seeds disjoint from training seeds (Section V) guarantees the two controllers are compared on identical windows -- a fair paired comparison -- but it does not guarantee those windows are unseen data in the stronger sense a reader might assume from "held-out." This does not change the paper's conclusion, which rests on the difference *between* the two controllers on identical windows rather than on either controller's generalization to genuinely novel traffic, but it does mean this paper is not evidence about generalization, only about relative performance under repeated exposure to one fixed trace.
