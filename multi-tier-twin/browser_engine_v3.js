/* Multi-Tier Digital Twin v3 browser engine.
...
(() => {
  ...
  function browserConfig() {
    ...
    const profile=document.getElementById('prach_profile').value;
    const preset={moderate:[1200,20],severe:[3000,10],extreme:[7000,5]}[profile];
    const prach={enabled:value('prach_enabled')===1,profile,devices:preset?preset[0]:Math.max(1,value('prach_devices')),
      start:Math.max(1,value('prach_start')),duration:preset?preset[1]:Math.max(1,value('prach_duration')),
      horizon:Math.max(40,value('prach_horizon')),preambles:Math.max(4,value('prach_preambles')),
      raosPerSec:Math.max(1,value('prach_raos_sec')),maxAttempts:Math.max(1,value('prach_max_attempts')),
      background:Math.max(0,value('prach_background')),fixedAcb:clamp(value('prach_fixed_acb'),.005,1),
      fixedBarring:Math.max(1,value('prach_fixed_barring')),fixedBackoff:Math.max(1,value('prach_fixed_backoff')),
      ruleHigh:clamp(value('prach_rule_high'),.05,.95),ruleLow:clamp(value('prach_rule_low'),0,.9),
      demandTarget:clamp(value('prach_demand_target'),.1,1.5),episodes:Math.max(1,value('train_episodes'))};
    if(prach.start+prach.duration>=prach.horizon) throw new Error('PRACH storm must end before the evaluation horizon.');
    if(prach.ruleLow>=prach.ruleHigh) throw new Error('PRACH recovery threshold must be below the high-collision threshold.');
    return {scenario, area, basics, advanced:Object.fromEntries(TIER_DEFAULTS.map(t=>[t.tier,readAdvanced(t.tier)])),
      ...
      wifiOffload:{enabled:value('wifi_offload_enabled')===1,discovery:clamp(value('wifi_discovery_probability')),
        authentication:clamp(value('wifi_auth_probability')),minRssi:value('wifi_min_rssi_dbm'),loadCeiling:clamp(value('wifi_load_ceiling_pct')/100)},prach,
      demandFollow:{minimum:[value('df_embb_min'),value('df_urllc_min'),value('df_miot_min')],
        strength:clamp(value('df_strength')),smoothing:clamp(value('df_smoothing'),0,.95)},
  ...
  function calculatedPreview(c) {
    ...
    out.plan.capacity={network_mbps:sum(tiers,'tier_capacity_mbps'),network_loaded_mbps:sum(tiers,'loaded_tier_capacity_mbps'),
      ...
      basis:'JS v3 aggregate calculation: theoretical peak and interference/rain-derated loaded capacity'};
  ...
  function evaluate(c, preview, policy, seed, learned=null) {
    ...
    let last=0,previousDemandAlloc=[.55,.25,.20];
    for(let t=0;t<c.steps;t++) {
      const s=stateAt(c,random,t); let alloc;
      if(policy==='static') alloc=TEMPLATES[0];
      else if(policy==='demand_follow') {
        const mins=c.demandFollow.minimum.map(x=>Math.max(0,x)),minTotal=mins.reduce((a,b)=>a+b,0);
        if(minTotal>=1) throw new Error('Demand-follow minimum slice shares must total less than 1.');
        const demand=s.slice(0,3),dz=demand.reduce((a,b)=>a+b,0),remaining=1-minTotal;
        const target=mins.map((m,i)=>m+remaining*demand[i]/Math.max(1e-9,dz));
        const responsive=target.map((x,i)=>previousDemandAlloc[i]*(1-c.demandFollow.strength)+x*c.demandFollow.strength);
        alloc=responsive.map((x,i)=>previousDemandAlloc[i]*c.demandFollow.smoothing+x*(1-c.demandFollow.smoothing));
        const az=alloc.reduce((a,b)=>a+b,0);alloc=alloc.map(x=>x/az);previousDemandAlloc=alloc;
      }
    ...
    const wifi=tiers.find(t=>t.tier==='wifi7_indoor'),w=c.wifiOffload;
    const wifiRssi=wifi?-82+12*wifi.area_covered+c.wifiBias:-Infinity;
    const wifiLoad=wifi?clamp(sums.served[0]/Math.max(1,wifi.loaded_tier_capacity_mbps*n)):1;
    const eligible=w.enabled&&wifi&&wifiRssi>=w.minRssi&&wifiLoad<=w.loadCeiling;
    const offeredSessions=Math.round(c.sessions*scenarioMix(c.scenario)[0]);
    const offloaded=eligible?Math.round(offeredSessions*c.indoor*w.discovery*w.authentication):0;
    if(wifi&&offeredSessions>0) share.wifi7_indoor=offloaded/offeredSessions;
    ...
    return {mean_reward:sums.reward/n,mean_satisfaction:Object.fromEntries(SLICES.map((s,i)=>[s,sums.sat[i]/n])),
      ...
      wifi_offload:{enabled:w.enabled,wifi_available:!!wifi,estimated_rssi_dbm:wifiRssi,estimated_load:wifiLoad,eligible,embb_candidates:offeredSessions,offloaded_sessions:offloaded,retained_on_cellular:offeredSessions-offloaded,offload_rate:offloaded/Math.max(1,offeredSessions)},
  ...
  function comparisonForSeed(c, preview, seed) {
    ...
    const prach=c.prach.enabled?window.PrachV2.compare(c.prach,seed):null;
    if(prach) for(const k of POLICIES){const a=prach[k],r=results[k];r.prach=a;r.mean_satisfaction.mIoT*=a.successRate;r.sla_compliance.mIoT*=a.successRate;r.mean_access_latency_ms+=a.meanDelayMs/1000;r.mean_reward=.8*r.mean_reward+.2*a.successRate-.1*a.collisionRate-.1*a.failures/Math.max(1,a.successes+a.failures);}
  ...
  function buildComparison(c) {
    ...
    return {implementation:'multi_tier_v3_browser_js_aggregate',evidence_class:'synthetic_browser_screening',architecture:'centralized aggregate branching approximation',scenario:c.scenario,scenario_label:P.compare[c.scenario].scenario_label,scenario_description:P.compare[c.scenario].scenario_description,
      ...
      prach:c.prach.enabled?{config:c.prach,results:Object.fromEntries(POLICIES.map(k=>[k,results[k].prach]))}:null,
      feasibility:P.compare[c.scenario].feasibility,validation:validation(c,runs),limitations:['Browser v3 is an aggregate JavaScript screening model and is not numerically identical to the Python twin.','Synthetic sessions; not carrier evidence.','PRACH contention is modeled only for the cellular mIoT slice; mmWave small cells and Wi-Fi offload are excluded.','Wi-Fi offload uses aggregate discovery, RSSI, authentication-policy and load gates; it is not a packet-level 802.11 model.','Linear browser PPO/DQN approximations do not establish neural-agent convergence.','Propagation and handover outcomes are screening approximations; no ray tracing or RRC signalling simulation.']};
  }
  ...
  async function execute(kind) {
    ...
    if(kind==='compare'||kind==='all'){const comparison=buildComparison(c);render(comparison);renderPrach(comparison.prach,calculatedPreview(c));renderWifiOffload(comparison.results.rule_based.wifi_offload);}
  ...
  function install() {
    buildAdvancedRows();
    document.querySelectorAll('#controls input, #controls select, #advancedCfg input, #advancedCfg select, #prachCfg input, #prachCfg select').forEach(el=>el.disabled=false);
